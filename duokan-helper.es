...
class BookItem extends React.Component {
  ...
  render = () => {
    ...
    return ( < li className = {
      ...
      } < /div> < /div > < div className = "act" > {
        do {
          if (book.price != 0) {
            if (!!book.paid) { < span key = {
                  id()
                } > 已购买 < b className = "l" > < /b><b className="r"></b >
                < /span>
            } else if (!!book.carted) { < span key = {
                id()
              } > 已加入购物车 < /span>
            } else {
              /*
            ...
            if (!!book.paid) { < span key = {
              ...
              href = "<%= book.url %>"
              hidefocus = "hidefocus" > 去领取 < /a>
            }
          }
        }
      } { /*<span className="u-sep">|</span><a className="j-delete delete" href="javascript:void(0);" hidefocus="hidefocus">取消收藏</a>*/ } <
      /div> < /div > < div className = "mask j-mask" >
      < div className = "u-mask1" > < /div> < a className =
    "show j-restore"
    href = "javascript:void(0)"
    hidefocus = "hidefocus" > 恢复收藏 < /a> < /div > < /li>
  )
}
}
...
function getBookPromise(id) {
  return qwest.get(`${BASEPATH}/book/${id}`)
    .catch(errorHandler)
}
...
function getBookInfoByDuokanApiPromise(id) {
  return new Promise((resolve, reject) => {
    qwest.get(
        `http://www.duokan.com/hs/v0/android/store/book/${encodeURIComponent(id)}`
      )
...
function searchBookByDoubanApiPromise({
...
function createDoubanLink(title, url =
  `https://book.douban.com/subject_search?search_text=${encodeURIComponent(title)}`
) {
  return createElementByReact( < div > < a href = {
      url
    }
    target = "_blank" > 到豆瓣看大家对 {
      title
    }
    的评价 < /a></div > )
}
...
function createDoubanRating(rating) {
  return createElementByReact( < span > 豆瓣评分: {
      rating
    } < /span>)
  }
  ...
  function createAmazonLink(title) {
    return createElementByReact( < div > < a href = {
        `http://www.amazon.cn/s/${encodeURIComponent(title)}`
      }
      target = "_blank" > 到亚马逊购买 {
        title
      }
      的实体书 < /a></div > )
  }
  ...
  function singleHandler([, id]) {
    getBookPromise(id).then((xhr, [data]) => {
        ...
        getBookInfoByDuokanApiPromise(id)
          .then(({
            title, authors, translators, rights
          }) => ({
            title, authors, translators, publisher: rights
          }))
