        ...
        <div className="info">
          <div className="wrap">
            ...
            {do {
              if (book.price != 0) {
                [
                  <em key={id()}>Â¥ { (+book.price * 1.0).toFixed(2) }</em>,
                  !!book.new_price && <del key={id()}>Â¥ { (+book.old_price * 1.0).toFixed(2) }</del>
                ].filter((n) => n !== true)
              } else {
                <b key={id()}>免费</b>
              }
          ...
          <div className="act">
            {do {
              if (book.price != 0) {
                if (!!book.paid) {
                  <span key={id()}>å·²è´­ä¹°<b className="l"></b><b className="r"></b></span>
                } else if (!!book.carted) {
                  <span key={id()}>å·²åŠ å…¥è´­ç‰©è½¦</span>
                } else {
                  /*
                  [
                    <a key={id()} href="javascript:void(0)" className="j-cart" hidefocus="hidefocus">åŠ å…¥è´­ç‰©è½¦</a>,
                    <span key={id()} style={{display: 'none'}}>å·²åŠ å…¥è´­ç‰©è½¦</span>
                  ]
                  */
                }
              } else {
...
function getMinTimeline(timelines) {
  ...
  return _.reduceRight(timelines, (min, timeline) => min.Price > timeline.Price ? timeline : min) || {
    Price: NaN
  }
}
